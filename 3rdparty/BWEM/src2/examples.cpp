    ...
    for (int y = 0; y < theMap.Size().y; ++y)
        for (int x = 0; x < theMap.Size().x; ++x)
        {
            ...
            if (MapDrawer::showGroundHeight && (tile.GroundHeight() > 0))
            {
                ...
                if (tile.Doodad())
                    bw->drawTriangleMap(center(t) + Position(0, 5), center(t) + Position(-3, 2), center(t) + Position(+3, 2), Colors::White);
            }
        }
...
static void printNeutral(const Map &theMap, const Neutral *n, MapPrinter::Color col)
{
    ...
    for (int i = 0; i < stackSize; ++i)
    {
        ...
        if (!theMap.Valid(origin) || !theMap.Valid(origin + size - 1))
            break;
...
void printMap(const Map &theMap)
{
    ...
    for (int y = 0; y < theMap.WalkSize().y; ++y)
        for (int x = 0; x < theMap.WalkSize().x; ++x)
        {
            ...
            if (miniTile.Sea())
            {
                if (MapPrinter::showSeaSide && seaSide(p, &theMap))
                    col = MapPrinter::Color::seaSide;
                else
                    col = MapPrinter::Color::sea;
            }
            else
            {
...
{
    if (theMap.StartingLocations().size() < 2)
        return;
    ...
    while (b == a)
        b = WalkPosition(random_element(theMap.StartingLocations()));
...
//      b = WalkPosition(theMap.RandomPosition());
    ...
    else                                                                        // at least one ChokePoint between a and b:
    {
        ...
        {
            if (cpPrevious)
                MapPrinter::Get().Line(cpPrevious->Center(), cp->Center(), col, MapPrinter::dashed);
