...
static bool canBuildWall(const Map &theMap, BWAPI::UnitType type, TilePosition location)
{
    ...
    if (!theMap.Valid(location))
        return false;

    if (!theMap.Valid(location + dim - 1))
        return false;
...
static bool tightEnough(UnitType type, const BorderTileInfo &BorderTile)
{
    ...
    {
        if (BorderTile.TopBorder())
            return true;

        if (BorderTile.RightBorder())
            return true;

        if (BorderTile.BottomBorder() && BorderTile.LeftBorder())
            return true;
                          ...
                          const vector<TilePosition> &BuildableBorderTiles2)
{
    ...
    for (const TilePosition &t : BuildableBorderTiles1)
        map_BorderTile_Info.emplace(t, BorderTileInfo(*m_theMap, t));

    for (const TilePosition &t : BuildableBorderTiles2)
        map_BorderTile_Info.emplace(t, BorderTileInfo(*m_theMap, t));
        ...
        for (const TilePosition &borderTile2 : BuildableBorderTiles2)
            if (abs(borderTile1.y - borderTile2.y) < wallMaxDim.y)
                if (abs(borderTile1.x - borderTile2.x) < wallMaxDim.x)
                    ...
                    {
                        if (!tightEnough(m_BuildingTypes.front(), map_BorderTile_Info.at(borderTile1)))
                            continue;

                        if (!tightEnough(m_BuildingTypes.back(), map_BorderTile_Info.at(borderTile2)))
                            continue;
                            ...
                            for (int dx1 = 0; dx1 < BuildingDims.front().x; ++dx1)
                                for (int dy2 = 0; dy2 < BuildingDims.back().y; ++dy2)
                                    for (int dx2 = 0; dx2 < BuildingDims.back().x; ++dx2)
                                    {
                                        ...
                                        if (!canBuildWall(*m_theMap, m_BuildingTypes.front(), m_Locations.front()))
                                            continue;

                                        if (wallSize >= 2)
                                        {
                                            if (!canBuildWall(*m_theMap, m_BuildingTypes.back(),  m_Locations.back()))
                                                continue;

                                            if (overlap(m_Locations.front(), BuildingDims.front(),
                                                        m_Locations.back(), BuildingDims.back()))
                                                continue;
                                        ...
                                        for (int y = minY; y <= maxY; ++y)
                                            for (int x = minX; x <= maxX; ++x)
                                            {
                                                if (wallSize == 3)
                                                {
                                                    ...
                                                    if (middleBuildingHasBorderTiles)
                                                        continue;

                                                    if (!canBuildWall(*m_theMap, m_BuildingTypes[1], m_Locations[1]))
                                                        continue;

                                                    if (overlap(m_Locations[1], BuildingDims[1], m_Locations[0], BuildingDims[0]))
                                                        continue;

                                                    if (overlap(m_Locations[1], BuildingDims[1], m_Locations[2], BuildingDims[2]))
                                                        continue;

                                                    if (disjoint(m_Locations[1], BuildingDims[1], m_Locations[0], BuildingDims[0]))
                                                        continue;

                                                    if (disjoint(m_Locations[1], BuildingDims[1], m_Locations[2], BuildingDims[2]))
                                                        continue;
                                                }

                                                bool zealotTight = true;

                                                for (int b = 0; b < wallSize; ++b)
                                                    if (m_BuildingTypes[b] == Terran_Barracks)
                                                ...
                                                if (m_tight == tight_t::zergling)
                                                {
                                                    ...
                                                    for (int d1 = 0; d1 < wallSize; ++d1)
                                                        if (m_BuildingTypes[d1] == Terran_Supply_Depot)
                                                            for (int d2 = d1 + 1; d2 < wallSize; ++d2)
                                                                if (m_BuildingTypes[d2] == Terran_Supply_Depot)
...
void ExampleWall::Compute()
{
    ...
    if (!Possible())
        Compute(m_cp->GetAreas().first);

    if (!Possible())
        Compute(m_cp->GetAreas().second);
